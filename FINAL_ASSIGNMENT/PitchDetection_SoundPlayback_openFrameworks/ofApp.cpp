...
void ofApp::setup(){
   ...
   // ofSoundStreamListDevices();
    ...
    backGroundNoise.emplace_back(new Key(40, 0.5, 0.2));
    backGroundNoise.emplace_back(new Key(40, 0.5, 0.9));
    backGroundNoise.emplace_back(new Key(100, 0.6));

    // Memory notes initialisation
    for(int i = 0 ; i < 10 ; i++){
        memoryNoise.emplace_back(new Key(10, 0.8));
    }

    // Live notes mimic initialisation
    for(int i = 0 ; i < 10 ; i++){
        keys.emplace_back(new Key(10, 1));
    }
...
void ofApp::update(){
        ...
        if(!notesInTime.empty()){
            ...
            for(int i = 0 ; i < 10 ; i++){
                if(notesInTime.front().size() > 0) {
                    ...
                    int n = ofMap(notesInTime.front()[i], 21, 108, 0, 3000 );
                    keys[i]->freq(n);
    ...
    // If mike is off, turn all keys off
        ...
        if(!global.empty() && !sPressed){
            unsigned int ref = ofRandom(0, 5);
            unsigned int rd = ofRandom(0,5);
            if (ref == rd) {
            ...
            if(!memoryNotes.empty()){
                ...
                for(int i = 0 ; i < 10 ; i++){
                    ...
                    if(memoryNotes.front().size() > 0){
                        int n = ofMap(memoryNotes.front()[i], 21, 108, 0, 4000 );
    ...
    if(receiver.hasWaitingMessages() || sPressed){
        ...
        if(m.getAddress() == "/buttonOn" || sPressed){
            ...
            char nbNotes = (char)notes.size();
            serial.writeByte(nbNotes);
...
void ofApp::audioOut(float * output, int bufferSize, int nChannels) {
    for(unsigned i = 0 ; i < bufferSize ; i++) {
       ...
       // currentFrame += backGroundNoise[2]->play(1);
            if(isPlaying){
