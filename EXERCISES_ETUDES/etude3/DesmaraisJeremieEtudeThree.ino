...
void live()
{
   ...
   if(note > 8){
     tone(BUZZER_PIN,note+offsetFrequency, 100);
...
void startUpTimer()
{
  ...
  startTime = millis();
  // Reset timePassed so it starts now
  timePassed = 0;
...
void updateTimer()
{
  // Set the timepassed for now: the current time (millis()) minus the startTime
...
{
    // Just play the note adding the photoCell offset
...
{
  notes[countNotes] +=  averageOffsetFreq; // Add the averageOffset to the current note
  durations[countNotes] = timePassed;       // Record the duration in the array
  countNotes++;                             // increment the counter
  activeNoteButton = false;                 // Since the button is no longer pressed: = false
...
{ 
  ...
  return map(analogRead(PHOTO_PIN), 400, 800, 0, 1000);
}
...
{ 
    ...
    // Read the photo signal
   int rawSenseVal = analogRead(PHOTO_PIN);
   runningAverageBuffer[nextCount] = rawSenseVal;  // add into the array...
   nextCount++;
   if (nextCount >= RUNNING_SAMPLES){              // ... until the array is full
    nextCount = 0; 
   }   
   int currentSum= 0;                              // Prepare return value
   for(int i=0; i< RUNNING_SAMPLES; i++){
    currentSum+= runningAverageBuffer[i];          // add all of the runningAvg...
   }
   return currentSum / RUNNING_SAMPLES;            // ...Return the average.
...
void playWithDuration()
{
  ...
  for (int i = 0; i < countNotes; i++)
  {
    ...
    // Play the note with duration array
    tone(BUZZER_PIN, notes[i], durations[i]);
    

    delay(durations[i]); // wait for tone to finish... yes.
    //check if mode button is pressed
    if (digitalRead(BUTTON_MODE_PIN) == HIGH) {
      break;
    }
